...
#define VULKAN_RATE_TEXTURE_FILL_PARALLEL_FETCHES       (16)
#define VULKAN_RATE_TEXTURE_FILL_WORKGROUP_SIZE         (256)
#define VULKAN_RATE_TEXTURE_FILL_STARTING_WORKGROUPS    (16)
#define VULKAN_RATE_TEXTURE_FILL_STARTING_LOOP_COUNT    (1024)
#define VULKAN_RATE_TEXTURE_FILL_TARGET_TIME_US         (250000)
#define VULKAN_RATE_TEXTURE_FILL_WARMUP_RUNS            (3)
#define VULKAN_RATE_TEXTURE_FILL_MAX_TEXTURE_SIDE       (8192)
...
typedef struct vulkan_rate_texture_fill_uniform_buffer_t {
    ...
    uint32_t padding[3];
} vulkan_rate_texture_fill_uniform_buffer;
...
static const char *_VulkanRateTextureFillGetOperationName(uint32_t operation);
...
static test_status _VulkanRateTextureFillRegisterSubtest(uint32_t operation, const char *test_name) {
    return VulkanRunnerRegisterTest(&_VulkanRateTextureFillEntry, (void *)(uint64_t)operation, test_name, TESTS_VULKAN_RATE_TEXTURE_FILL_VERSION, false);
}

static test_status _VulkanRateTextureFillEntry(vulkan_physical_device *physical_device, void *config_data) {
    test_status status = TEST_OK;
    uint32_t operation = (uint32_t)(uint64_t)config_data;
    const char *shader_name = _VulkanRateTextureFillGetShaderName(operation);
    const char *operation_name = _VulkanRateTextureFillGetOperationName(operation);
    ...
    uint32_t maximum_texture_side = min(device.physical_device->physical_properties.properties.limits.maxImageDimension2D, VULKAN_RATE_TEXTURE_FILL_MAX_TEXTURE_SIDE);
    ...
    status = VulkanShaderAddFixedSizeDescriptor(&shader, sizeof(vulkan_rate_texture_fill_uniform_buffer), "uniform buffer", VULKAN_BINDING_UNIFORM, 0, 2);
    if (!TEST_SUCCESS(status)) {
        goto cleanup_shader;
    }
    ...
    status = VulkanMemoryAddRegion(&uniform_memory, sizeof(vulkan_rate_texture_fill_uniform_buffer), "uniform buffer", VULKAN_REGION_UNIFORM, VULKAN_REGION_NORMAL);
    if (!TEST_SUCCESS(status)) {
    ...
    if (MainGetTestResultFormat() == test_result_csv) {
        LOG_PLAIN("%s,\n", physical_device->physical_properties.properties.deviceName);
        LOG_PLAIN("Operation,Texture Edge,Texture Fill Rate (GTexel/s),Workgroups,Loops\n");
    }
    ...
    for (uint32_t texture_side = 1; texture_side <= maximum_texture_side; texture_side <<= 1) {
        ...
        if (MainGetTestResultFormat() == test_result_csv) {
            LOG_PLAIN("%s,%lu,%.3f,%llu,%llu\n", operation_name, texture_side, (double)top_result / 1000000000.0, top_workgroups, top_loops);
            ...
            INFO("Texture fill rate for %s %lux%lu: %.3f %sTexel/s (workgroups: %llu, loops: %llu)\n", operation_name, texture_side, texture_side, texel_conversion.value, texel_conversion.units, top_workgroups, top_loops);
        }
        result_index++;
    }
...
static test_status _VulkanRateTextureFillRunSide(vulkan_device *device, vulkan_compute_pipeline *pipeline, vulkan_memory *uniform_memory, vulkan_command_sequence *command_sequence, uint32_t texture_side, uint32_t operation, uint64_t *top_result, uint64_t *top_workgroups, uint64_t *top_loops) {
    ...
    status = VulkanMemoryAddRegion(&memory, VULKAN_RATE_TEXTURE_FILL_WORKGROUP_SIZE * sizeof(float) * 4, "dummy outputs", VULKAN_REGION_STORAGE, VULKAN_REGION_NORMAL);
    if (!TEST_SUCCESS(status)) {
        goto cleanup_memory;
    }
    ...
    for (uint32_t i = 0; i < VULKAN_RATE_TEXTURE_FILL_WARMUP_RUNS; i++) {
        status = _VulkanRateTextureFillExecuteKernel(VULKAN_RATE_TEXTURE_FILL_STARTING_WORKGROUPS, VULKAN_RATE_TEXTURE_FILL_STARTING_WORKGROUPS, texture_side, NULL, NULL, device, uniform_region, command_sequence, pipeline);
        if (!TEST_SUCCESS(status)) {
            goto free_memory;
        }
    }
    ...
    *top_loops = 0;
    *top_workgroups = 0;
    uint64_t workgroup_count = VULKAN_RATE_TEXTURE_FILL_STARTING_WORKGROUPS;

    while (true) {
        uint64_t result = 0;
        uint64_t time_taken = 0;
        uint32_t loop_count = VULKAN_RATE_TEXTURE_FILL_STARTING_LOOP_COUNT;

        while (time_taken < VULKAN_RATE_TEXTURE_FILL_TARGET_TIME_US) {
            status = _VulkanRateTextureFillExecuteKernel(workgroup_count, loop_count, texture_side, &result, &time_taken, device, uniform_region, command_sequence, pipeline);
            if (!TEST_SUCCESS(status)) {
                goto free_memory;
            }
        ...
        if (loop_count == VULKAN_RATE_TEXTURE_FILL_STARTING_LOOP_COUNT * 2) {
            break;
        }
        workgroup_count *= 2;
    }
...
static test_status _VulkanRateTextureFillFillTexture(vulkan_texture *texture, uint32_t texture_side) {
    ...
    uint64_t rng_state;
    HelperSeedRandom(&rng_state, VULKAN_RATE_TEXTURE_FILL_RNG_SEED);
...
static test_status _VulkanRateTextureFillExecuteKernel(uint64_t workgroup_count, uint32_t loop_count, uint32_t texture_side, uint64_t *result, uint64_t *time_taken, vulkan_device *device, vulkan_region *uniform_region, vulkan_command_sequence *command_sequence, vulkan_compute_pipeline *pipeline) {
    ...
    uniform_buffer_memory->texture_side_shift = _VulkanRateTextureFillPowerOfTwoLog2(texture_side);
    ...
    if (time == 0) {
        ...
        uint64_t total_fetches = (uint64_t)groups_x * (uint64_t)groups_y * (uint64_t)groups_z * VULKAN_RATE_TEXTURE_FILL_WORKGROUP_SIZE * VULKAN_RATE_TEXTURE_FILL_PARALLEL_FETCHES * loop_count;
...
static const char *_VulkanRateTextureFillGetShaderName(uint32_t operation) {
    switch (operation) {
    case VULKAN_RATE_TEXTURE_FILL_OP_TEXEL_FETCH:
